...
function updateProxyCount() {
    if (!proxyListTextarea || !proxyCountElement || !totalProxiesStat) return;
    
    const text = proxyListTextarea.value.trim();
    const lines = text ? text.split('\n').filter(line => line.trim() !== '') : [];
    proxyCountElement.textContent = `代理数: ${lines.length}`;
    if (proxyCountBadge) proxyCountBadge.textContent = lines.length;
    totalProxiesStat.textContent = lines.length;
}

// 更新直播源计数
function updateStreamCount() {
    if (!streamListTextarea || !streamCountElement || !totalStreamsStat) return;
    ...
    streamCountElement.textContent = `直播源: ${count}`;
    if (streamCountBadge) streamCountBadge.textContent = count;
    ...
    while (testQueue.length > 0 && !stopTesting) {
        ...
        testSingleProxy(proxy, currentTestId).then(result => {
            ...
            if (result) {
                ...
                updateProxyResultsBadge(results.length);
...
async function testAllStreamsForProxy(proxy, streams) {
    ...
    addLog('直播源测试', `开始通过代理 ${proxy} 测试 ${total} 个直播源`, 'info');
        ...
        for (const stream of batch) {
            ...
            const testPromise = testStreamWithProxy(stream.url, proxy, currentTestId++).then(result => {
                if (result) {
                    ...
                    addLog('测试', 
                        `代理 ${proxy} -> 直播源 ${stream.original_text} 测试${result.success ? '成功' : '失败'}`, 
                        result.success ? 'success' : 'error');
                }
...
function updateProxyCards() {
    ...
    proxies.forEach(proxy => {
        ...
        html += `
            <div class="proxy-card">
                <div class="proxy-card-header">
                    <div class="proxy-card-title">${proxy}</div>
                    <div class="proxy-card-stats">成功: ${successfulStreams.length} / 失败: ${failedStreams.length}</div>
                </div>
        ...
        if (successfulStreams.length > 0) {
            successfulStreams.forEach((stream, index) => {
                html += `
    <div class="stream-item">
        <div class="stream-url">${stream.stream_url}</div>
        
        <div class="stream-info">
            <span class="stream-group">${stream.group || '未分组'}</span>
            <span class="stream-status">${stream.response_time}ms${stream.redirect_count > 0 ? ` (${stream.redirect_count}次跳转)` : ''}</span>
        </div>
    </div>
`;
    ...
    proxyCardsContainer.innerHTML = html;
    
    const matchRate = totalStreams > 0 ? Math.round((totalMatched / totalStreams) * 100) : 0;
    
    if (totalStreamsStat) totalStreamsStat.textContent = totalStreams;
    if (matchedStreamsStat) matchedStreamsStat.textContent = totalMatched;
    if (bestProxyStat) bestProxyStat.textContent = bestProxy ? (bestProxy.length > 20 ? bestProxy.substring(0, 20) + '...' : bestProxy) : '-';
    if (matchRateStat) matchRateStat.textContent = `${matchRate}%`;
    updateStreamResultsBadge(totalStreams);
}
...
async function startFullTest() {
    ...
    try {
        ...
        addLog('系统', `找到 ${availableProxies.length} 个可用代理，开始测试直播源`, 'success');
        
        updateTestStatus('测试直播源中...', 'warning');
        addLog('系统', '第二步：开始测试直播源', 'info');
        ...
        for (const proxy of availableProxyAddresses) {
            ...
            completedProxies++;
            const progress = 50 + (completedProxies / availableProxyAddresses.length) * 50;
...
function updateHistoryDisplay() {
    ...
    testHistory.forEach((item, index) => {
        ...
        const testType = isFullTest ? 'SOCKS5直播源测试' : 'SOCKS5验证';
