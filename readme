# 📺 Live 频道资源站

[![站点状态](https://img.shields.io/website?down_message=Down&up_message=Online&url=https%3A%2F%2Flive.404003.xyz)](https://live.404003.xyz)
[![关注公众号](https://img.shields.io/badge/%E5%85%B3%E6%B3%A8-WX%E5%85%AC%E4%BC%97%E5%8F%B7-brightgreen.svg)](https://live.404003.xyz)

---

> **免责声明 (Disclaimer)**
>
> 本站提供的直播源均采集自互联网，**仅供学习与交流使用**。
> **请勿用于任何商业用途。** 如有侵权或违规内容，请及时联系我进行处理。

---

## 🌐 核心服务 (Core Service)

### 🚀 资源站主页

您的专属直播源集合入口。

* **主站域名：** **`https://live.404003.xyz`**
* **示例链接：** `https://live.404003.xyz/m3u/湖南移动.m3u`

---

## 🎨 实用工具 (Utility Tools)

### 1. 🖼️ 电视台 Logo 整合

为您的 IPTV 播放器等平台提供标准化的台标资源。

* **调用方式：** `https://live.404003.xyz/LOGO/{name}.png`
* **使用说明：** 将 `{name}` 替换为频道名称（如 `CCTV1`、`湖南卫视` 等）。
* **适用场景：** 支持自定义调用，适用于 **IPTV 播放器**、**EGP 列表** 等。

### 2. 🔄 直播源格式转换工具

快速实现 `txt` 与 `m3u` 格式之间的互转。

* **在线地址：** `https://live.404003.xyz/txt2m3u.html`
* **特点：**
    * 支持 **txt** 和 **m3u** 格式互转。
    * 单文件 HTML 网页，支持将网页源代码保存到本地**离线使用**。

---

## 🔗 代理订阅服务 (Proxy Subscriptions)

提供特定平台的代理聚合直播源，方便统一管理。

| 平台 (Platform) | 订阅地址 (Subscription URL) | 备注 (Note) |
| :--- | :--- | :--- |
| **斗鱼一起看** | `https://php.404003.xyz/live/douyuyqk.php` | 聚合斗鱼内容 |
| **虎牙一起看** | `https://php.404003.xyz/live/huyayqk.php` | 聚合虎牙内容 |
| **YY轮播** | `https://php.404003.xyz/live/yylunbo.php` | 聚合 YY 轮播内容 |

---

## 🤝 保持联系 (Stay Connected)

### 📣 获取更多资源与技术分享

欢迎关注我的微信公众号 **【网络志】**，不错过任何实用网络资源与最新技术分享！

![微信公众号二维码](LOGO/wlz.jpg)

---

## ⭐ Star History (Optional)

如果您觉得本项目对您有帮助，请给一个 Star 鼓励！

[![Star History Chart](https://api.star-history.com/svg?repos=wind005/TVlive&type=Date)](https://star-history.com/#wind005/TVlive&Date)